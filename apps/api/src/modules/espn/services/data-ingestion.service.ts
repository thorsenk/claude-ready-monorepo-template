  ...
  private async processLeagueData(
    ...
    await this.prisma.leagueSettings.upsert({
      ...
      create: {
        ...
        qbScoring: JSON.stringify(internalLeague.scoringSettings),
        rosterPositions: JSON.stringify(internalLeague.rosterPositions),
    ...
    const leagueRecord = await this.prisma.league.findUnique({ 
      where: { espnLeagueId: leagueId },
      select: { id: true }
    });
    ...
    if (leagueRecord) {
      await this.prisma.season.upsert({
        ...
        create: {
          leagueId: leagueRecord.id,
          year: seasonYear,
          startDate: new Date(`${seasonYear}-09-01`), // Approximate season start
          endDate: new Date(`${seasonYear}-12-31`), // Approximate season end
          isComplete: seasonYear < new Date().getFullYear(),
        },
        update: {
          isComplete: seasonYear < new Date().getFullYear(),
          updatedAt: new Date(),
    ...
    for (const espnTeam of espnTeams) {
      ...
      const team = await this.prisma.team.upsert({
        ...
        create: {
          ...
          coOwnerNames: JSON.stringify(internalTeam.coOwnerNames),
        },
        update: {
          name: internalTeam.name,
          logoUrl: internalTeam.logoUrl,
