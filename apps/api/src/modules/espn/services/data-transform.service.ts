...
export class DataTransformService {
  ...
  private mapLeagueType(espnType: string): 'standard' | 'keeper' | 'dynasty' | 'redraft' {
    const typeMap: Record<string, 'standard' | 'keeper' | 'dynasty' | 'redraft'> = {
      'STANDARD': 'standard',
      'KEEPER': 'keeper',
      'DYNASTY': 'dynasty',
      'REDRAFT': 'redraft',
  ...
  private mapScoringType(espnScoringType: number): 'standard' | 'ppr' | 'half_ppr' | 'custom' {
  ...
  private mapWaiverType(espnWaiverType: string): 'rolling' | 'continuous' | 'free_agent' {
    const waiverMap: Record<string, 'rolling' | 'continuous' | 'free_agent'> = {
  ...
  private mapDraftType(espnDraftType: string): 'snake' | 'auction' | 'linear' {
    const draftMap: Record<string, 'snake' | 'auction' | 'linear'> = {
      'SNAKE': 'snake',
      'LINEAR': 'linear',
      'AUCTION': 'auction',
    };
  ...
  private mapMatchupType(playoffTierType?: string): 'regular' | 'playoff' | 'consolation' | 'championship' {
    ...
    if (playoffTierType === 'CHAMPIONSHIP') return 'championship';
    return 'regular';
  }

  private mapTransactionType(espnType: string): 'free_agent' | 'trade' | 'waiver' | 'drop' | 'add' {
    const typeMap: Record<string, 'free_agent' | 'trade' | 'waiver' | 'drop' | 'add'> = {
      'TRADE': 'trade',
      'WAIVER': 'waiver',
    ...
    return typeMap[espnType] || 'free_agent';
  }

  private mapTransactionStatus(transaction: ESPNTransaction): 'pending' | 'completed' | 'failed' | 'vetoed' {
    if (transaction.isPending) return 'pending';
    if (transaction.isSuccessful === false) return 'failed';
    return 'completed';
  }

  private mapTransactionItemType(itemType: string): 'acquired' | 'dropped' | 'traded_for' | 'traded_away' {
    const itemMap: Record<string, 'acquired' | 'dropped' | 'traded_for' | 'traded_away'> = {
      'ADD': 'acquired',
      'DROP': 'dropped',
      'TRADE_FOR': 'traded_for',
      'TRADE_AWAY': 'traded_away',
    };
    return itemMap[itemType] || 'acquired'; // Default to 'acquired' if unknown
  }
