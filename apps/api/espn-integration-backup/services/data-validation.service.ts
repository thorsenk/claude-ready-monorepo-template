...
} from '@claude-template/types/validation/espn-validation.schemas';
import {
  ESPNLeagueResponse,
  ESPNPlayerResponse,
  ESPNMatchupResponse,
...
export class DataValidationService {
  ...
  calculateQualityScore(validationResult: ValidationResult): number {
    ...
    const violations = validationResult.violations?.length || 0;
    const outliers = validationResult.outliers?.length || 0;
